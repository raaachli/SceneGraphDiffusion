...
import SceneGraph.box_utils as box_utils
from SceneGraph.graph import GraphTripleConv, GraphTripleConvNet
from SceneGraph.layout import boxes_to_layout, masks_to_layout
from SceneGraph.layers import build_mlp
from SceneGraph.crn import RefinementNetwork


class SgEmbedding(nn.Module):
  def __init__(self, vocab, image_size=(64, 64), embedding_dim=32,
               gconv_dim=64, gconv_hidden_dim=128,
    ...
    self.layout_noise_dim = layout_noise_dim  # default0

    num_objs = len(vocab['object_idx_to_name'])
    num_preds = len(vocab['pred_idx_to_name'])
    self.obj_embeddings = nn.Embedding(num_objs + 1, embedding_dim)  # 178
    self.pred_embeddings = nn.Embedding(num_preds, embedding_dim)  # 45

    if gconv_num_layers == 0:
      self.gconv = nn.Linear(embedding_dim, gconv_dim)
    elif gconv_num_layers > 0:  # = 5
      gconv_kwargs = {
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    mp = self.map_obj_img(obj_vecs, obj_to_img)
  ...
  def map_obj_img(self, obj_vecs, obj_to_img):
    """
     Input:
     - obj_vecs: FloatTensor of shape (O, D)
    ...
    out = out.scatter_add(0, idx, obj_vecs)
