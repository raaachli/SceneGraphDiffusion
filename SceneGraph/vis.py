...
def draw_layout(vocab, objs, boxes, save_dir, save_name, masks=None, size=256,
                show_boxes=False, bgcolor=(0, 0, 0), bg=None):
  if bgcolor == 'white':
    bgcolor = (255, 255, 255)
  ...
  with torch.no_grad():
    ...
    if show_boxes:
      for i, obj in enumerate(objs):
        ...
        draw_box(box, colors[i], save_dir, save_name, name)


def draw_box(box, color, save_dir, save_name, text=None):
  """
  ...
  if text is not None:
    # text_rect = Rectangle((x0, y0), w, TEXT_BOX_HEIGHT, fc='none', alpha=0.5)
    # plt.gca().add_patch(text_rect)
    # tx = 0.5 * (x0 + x1)
    # ty = y0 + TEXT_BOX_HEIGHT / 2.0
    plt.text(x0, y0+1, text, va='center', ha='left')
